const { Project, Path } = require("paper");
const { Line, dot, dist, makePolyLines } = require("./line");
const { floatToString, pointToStr } = require("./board");
const { PaperOffset } = require('paperjs-offset');

function makeTrackPath(track, width) {
    return PaperOffset.offsetStroke(new Path(track), width * .5, {join: 'round', cap: 'round'});
}
